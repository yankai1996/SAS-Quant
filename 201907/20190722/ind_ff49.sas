...
/********************************************************************************
	Author/Altered by: 	Christopher Alan Miller, derived 99.5% from Ed deHaan
	Macro: 		ind_ff49
	
	Purpose: 	to assign Fama French industry codes

	Versions:	1.0 - 	11/18/15	  - original version	
				
				
	
	Notes:		49 Industry classification codes obtained from French's website in Nov. 2015
				http://http://mba.tuck.dartmouth.edu/pages/faculty/ken.french/Data_Library/det_49_ind_port.html
				Fama-French created a "Software" category from "Business Services" and "Computers"
--------------------------------------------
...
Outputs the original dataset with appended industry code information:
	...
	'&ind_code' 	= count variable of industry codes from 1 through 49
	'FF_IND'		= text variable with name of the fama-french industry
	'&bin_var.#'	= 49 individual binary variables, one for each industry
	'_&global'		= global macro variable to include call 49 industry dummies
...
%macro ind_ff49 (dset, outp, sic, bin_var, ind_code );
...
*36  Softw Software;
if &sic ge 7370 and &sic le 7372   then FF_IND='SOFTW';
if &sic ge 7375 and &sic le 7375   then FF_IND='SOFTW';
if &sic ge 7373 and &sic le 7373   then FF_IND='SOFTW';
if FF_IND='SOFTW' then &ind_code=36;
...
*37 Chips  Electronic Equipment;
	...
	if FF_IND='CHIPS' then &ind_code=37;
...
*38 LabEq  Measuring and Control Equipment;
	...
	if FF_IND='LABEQ' then &ind_code=38;
...
*39 Paper  Business Supplies;
	...
	if FF_IND='PAPER' then &ind_code=39;
...
*40 Boxes  Shipping Containers;
	...
	if FF_IND='BOXES' then &ind_code=40;
...
*41 Trans  Transportation;
	...
	if FF_IND='TRANS' then &ind_code=41;
...
*42 Whlsl  Wholesale;
	...
	if FF_IND='WHLSL' then &ind_code=42;
...
*43 Rtail  Retail ;
	...
	if FF_IND='RTAIL' then &ind_code=43;
...
*44 Meals  Restaraunts, Hotels, Motels;
	...
	if FF_IND='MEALS' then &ind_code=44;
...
*45 Banks  Banking;
	...
	if FF_IND='BANKS' then &ind_code=45;
...
*46 Insur  Insurance;
	...
	if FF_IND='INSUR' then &ind_code=46;
...
*47 RlEst  Real Estate;
	...
	if FF_IND='RLEST' then &ind_code=47;
...
*48 Fin    Trading;
	...
	if FF_IND='FIN' then &ind_code=48;
...
*49 Other  Almost Nothing;
  	...
  	if FF_IND='OTHER' then &ind_code=49;
	...
	if(&ind_code=49) then &bin_var.49=1;		else &bin_var.49=0;
run;	
