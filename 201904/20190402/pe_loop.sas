...
%let root=C:\TEMP\displace\201903\20190328;
libname pwd "C:\TEMP\displace\20190402";
...
%do dirindex=1 %to 10;
	...
	%end; %else %if &dirindex=7 %then %do;
		libname sprd "&root\5k-obs\truncate2.5-country-portyear";
		%let finalname=k5_truncate_country_portyear;
	%end; %else %if &dirindex=8 %then %do;
		libname sprd "&root\5k-obs\truncate2.5-portyear";
		%let finalname=k5_truncate_portyear;
	%end; %else %if &dirindex=9 %then %do;
		libname sprd "&root\5k-obs\winsor2.5-country-portyear";
		%let finalname=k5_winsor_country_portyear;
	%end; %else %do;
		libname sprd "&root\5k-obs\winsor2.5-portyear";
...
libname pwd "C:\TEMP\displace\20190402\PE-150";
