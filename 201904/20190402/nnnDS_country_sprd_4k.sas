...
%do i=1 %to 4;
	%if &i=1 %then %do;
		%let name=truncate1-country-portyear;
		%let wsby=country portyear;
		%let wstype=delete;
	%end; %else %if &i=2 %then %do;
		%let name=truncate1-portyear;
		%let wsby=portyear;
		%let wstype=delete;
	%end; %else %if &i=3 %then %do;
		%let name=winsor1-country-portyear;
		%let wsby=country portyear;
		%let wstype=winsor;
	%end; %else %do;
		%let name=winsor1-portyear;
		%let wsby=portyear;
		%let wstype=winsor;
	%end;

	%let pwd = "C:\TEMP\displace\20190402\5k-obs\&name";
	*x md &pwd;
	x cd &pwd;
	libname pwd &pwd;
...
data pwd.slope_summary1; set slope_summary; run;

data junk;
test="&wsby";
run;

%winsor(dsetin=agret1, dsetout=agret1, byvar=country portyear, vars=rhs, type=delete, pctl=1 99);
