...
data junk; set tmp1.k4_winsor_portyear;
if parameter="pe";
if lhs="vw_slop" and tstat<0 then sign=-1;
run;

data tmp1.best; set tmp1.k4_winsor_portyear;
if id in (3222, 3224, 3226);
run;

data tmp1.outstat2; set tmp1.outstat tmp2.outstat; run;


x cd "C:\TEMP\displace\20190410";
ods tagsets.tablesonlylatex file="best.tex"  (notop nobot) stylesheet="sas.sty"(url="sas"); proc print data=tmp1.best; run; quit;
