...
dm 'log;clear;';
%let pwd = "C:\TEMP\displace\20190313\pvgosprd25";
x md &pwd;
x cd &pwd;
libname pwd &pwd;
...
%winsor(dsetin=testdata, dsetout=testdata2, byvar=mthyr, vars=&rhs, type=delete, pctl=10 90);

proc sql;
create table testdata3 as
select country, mthyr, 1 as globe, &lhs-avg(&lhs) as &lhs, &rhs-avg(&rhs) as &rhs 
from testdata2
group by mthyr
;
quit;
...
%twoDtest(country_vw_sprd, retsprd, pvgo, output1);
%twoDtest(country_ew_slope, slope, pvgo, output1);
%twoDtest(country_vw_slope, slope, pvgo, output1);
