...
%winsor(dsetin=testdata, dsetout=testdata2, byvar=globe, vars=&lhs short irisk dvol, type=delete, pctl=1 99);
...
proc means data=daily.irisk;
by country;
var irisk_old;
run;


proc means data=daily.dvol;
by country;
var dvol_old;
run;*/

data junk; set daily.dvol;
logdvol=log(coalesce(dvol_old, dvol_sum)/1000);
proc means; run;
