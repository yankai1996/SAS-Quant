...
*%winsor(dsetin=testdata, dsetout=testdata2, byvar=&wsby, vars=&lhs &rhs, type=&wstype, pctl=5 95);

*%winsor(dsetin=testdata, dsetout=testdata2, byvar=&wsby, vars=&lhs pvgo2, type=&wstype, pctl=5 95);
*%winsor(dsetin=testdata2, dsetout=testdata2, byvar=&wsby, vars=pe, type=&wstype, pctl=2.5 97.5);

%winsor(dsetin=testdata, dsetout=testdata2, byvar=portyear, vars=&lhs, type=delete, pctl=5 95);
%winsor(dsetin=testdata2, dsetout=testdata2, byvar=portyear, vars=pvgo2, type=delete, pctl=5 95);
%winsor(dsetin=testdata2, dsetout=testdata2, byvar=portyear, vars=pe, type=delete, pctl=5 95);
...
%mend twoDtest;
*%twoDtest(vw_slope, slope, pe, delete, globe, junk);
...
%do j = 1 %to 1;
	...
	%do k = 1 %to 3;
		...
		*%mthtest(pvgo2, &wstype, &wsby, &i&j&k.1);
		/*%rhstest2(short, &wstype, &wsby, &i&j&k.1);
		%rhstest2(irisk, &wstype, &wsby, &i&j&k.2);
		%rhstest2(dvol, &wstype, &wsby, &i&j&k.3);
		%rhstest2(short irisk dvol, &wstype, &wsby, &i&j&k.4);*/
	%end;
%end;
...
libname pwd "C:\TEMP\displace\20190620";
...
%pvgo_pe(90, 10, 90, 10);
%rhstest(pvgo2, delete, portyear, 0001);
%rhstest(pe, delete, portyear, 0002);
%rhstest(pvgo2 pe, delete, portyear, 0003);
...
x cd "C:\TEMP\displace\20190620";
ods tagsets.tablesonlylatex file="final.tex"  (notop nobot) stylesheet="sas.sty"(url="sas"); proc print data=pwd.best_k4_winsor_country_portyear; run; quit;
