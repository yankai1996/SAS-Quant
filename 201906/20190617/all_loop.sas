...
%macro twoDtest2(lhsdata, lhs, rhs, wstype, wsby, output);
...
data testdata; merge testdata(in=a) daily.short daily.irisk daily.dvol daily.snipo daily.pbsprd;
by country portyear;
if a;
*if portyear>=1987;
globe=1;
...
%MYREG2DSE(&lhs, &rhs, country, mthyr, 0, testdata2, &output);
...
%macro rhstest2(rhs, wstype, wsby, ijkl);

%twoDtest2(ew_sprd, retsprd, &rhs, &wstype, &wsby, tout1);
%twoDtest2(vw_sprd, retsprd, &rhs, &wstype, &wsby, tout2);
%twoDtest2(ew_slope, slope, &rhs, &wstype, &wsby, tout3);
%twoDtest2(vw_slope, slope, &rhs, &wstype, &wsby, tout4);
...
%do j = 1 %to 2;
	...
	%do k = 1 %to 3;
		...
		*%mthtest(pvgo2, &wstype, &wsby, &i&j&k.2);
...
libname pwd "C:\TEMP\displace\20190617";
