...
proc import out=work.&data
	file="C:\TEMP\displace\201905\20190531\&data..csv" 
	dbms=csv replace;
getnames=yes;
run;
...
if country="CHANNEL ISLANDS" then delete;
...
data ag21; set ag21;
if pvgo1<=-5 or pvgo1>=10 then pvgo1=.;
if pvgo2<=-5 or pvgo2>=10 then pvgo2=.;
if pvgo3<=-5 or pvgo3>=10 then pvgo3=.;
...
if be1=. then pvgo2=.;
*/
if pe<=-250 or pe>=250 then pe=.;
*if pe<=-50 or pe>=100 then pe=.;
*if RD<=0 then pe=.;
*if be1=. then pe=.;
...
data tmp1.new_pe3_bk; set new_pe_bk; run;
...
data ag21; set ag21;
if PE1<=-250 or PE1>=250 then PE1=.;
if PE2<=-250 or PE2>=250 then PE2=.;
if PE3<=-250 or PE3>=250 then PE3=.;
if PE4<=-250 or PE4>=250 then PE4=.;
if country~="";
keep code country portyear PE:;
run;

proc corr data=ag21; var PE:; run;
proc means data=ag21; run;
...
by country portyear;
var PE1;
output out=new_pe1_bk pctlpre=dec pctlpts=0 to 100 by 5;
run;

data bk.new_pe1_bk; set new_pe1_bk; run;
