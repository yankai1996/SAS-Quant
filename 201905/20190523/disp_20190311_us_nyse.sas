...
proc sql;
	create table agret2 as
	select a.*, b.emp as lagemp, a.emp-lagemp as empg,
	c.emp as lag2emp, d.emp as lag3emp, mean(lagemp,lag2emp,lag3emp) as empbar
	from agret1 as a
	left join agret1 as b on a.code=b.code and a.mthyr=b.mthyr+100
	left join agret1 as c on a.code=c.code and a.mthyr=c.mthyr+200
	left join agret1 as d on a.code=d.code and a.mthyr=d.mthyr+300
...
keep code mthyr portyear country ret emp: lag: rhs: globe n;
run;
...
%zscore(agret3, globe, mthyr, rhsemp, agret5);
