...
%macro makeEMP(input, output, absolute=1);

data emp; set &input;
keep code portyear emp cog_us sga_us;
if emp=0 then emp=.;
...
by code;
lag_emp=lag(emp);
lag_cog_us=lag(cog_us);
lag_sga_us=lag(sga_us);
if first.code then do;
	lag_emp=.;
	lag_cog_us=.;
	lag_sga_us=.;
end;
%if &absolute=1 %then %do;
...
EMP2=delta_emp/lag_cog_us;
EMP3=delta_emp/lag_sga_us;
*EMP2=delta_emp/COG;
*EMP3=delta_emp/SGA;
*EMP2=delta_emp/SL;
*EMP3=delta_emp/TA;
