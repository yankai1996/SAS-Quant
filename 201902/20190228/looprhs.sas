...
proc sql;
	...
	select country, mean(retsprd) as bar, std(retsprd) as sigma, mean(retsprd)/std(retsprd)*sqrt(12) as sharpe
	from sprd10
	group by country
;
quit;

data pwd.sprd_rhs&i; set sprd10; run;
%end;
data outp; set mysum:;
proc sort; by country;
run;

data pwd.outp; set outp; run;
